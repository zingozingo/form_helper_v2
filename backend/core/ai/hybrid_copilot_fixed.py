...
class HybridCopilot:
    ...
    async def get_response(self, question: str, field_context: Optional[Dict[str, Any]] = None, form_context: Optional[Dict[str, Any]] = None):
        ...
        Tries hardcoded knowledge first, then falls back to AI API if needed.
        ...
        # If no hardcoded response, try API
        try:
            if self.api_key:
...
Your goal is to be genuinely helpful by providing clear, contextual information about form fields."""
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            try:
                ...
                async with session.post(
                    ...
                    json={
                        "model": "claude-3-haiku-20240307",  # Update to newer model
                        "messages": [
                            {"role": "user", "content": prompt}
                        ],
                        "max_tokens": 500,
    ...
    async def test_api_connection(self):
        ...
        if self.api_provider == "openai" and not self.api_key.startswith("sk-"):
